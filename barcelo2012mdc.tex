...
\begin{IEEEkeywords}
\Az{Medium Access Control, decentralized constraint Satisfaction solver, learning MAC protocol}
\end{IEEEkeywords}
...
\AzDel{The random assignment of channel resources to wireless stations appears repeatedly in the literature (e.g., \cite{szpankowski1983asc}, \cite{kartsakli2011tmd}) \AzCom{Do we need to cite these?}.
This is roughly equivalent to allowing the wireless stations to randomly choose among the existing resources.
Obviously, it may happen that the same resource is assigned to more than one station or that more than one station randomly chooses the same resource.
This event is called a collision.
...
\AzDel{Recently, the initial idea of letting the stations randomly select one of the possible resources has been enhanced to contemplate several rounds (See \cite{fang2011dlm} and references therein).
...
Collisions seriously impair the network performance, specially when they trigger the auto rate fallback (ARF) mechanism \cite{martorell2012pec, martorell2012tfl} \AzCom{Do we need to cite these?}.}

\AzDel{The assignment of channel resources to wireless stations is just an instance of a constraint satisfaction problem (CSP). A CSP with $N$ variables is defined as follows (adopting the notation from \cite{duffy2011dcs}).
...
\AzDel{In radio resource allocation problems, it is natural that each of the variables represents one of the wireless stations, and that $\mathbf{B}$ represents the available resources.
It is likely that the different stations cannot communicate until the CSP is solved.
Therefore, it is necessary to find a solution in a distributed fashion, thus the problem becomes a decentralized CSP \cite{duffy2011dcs}.}

\AzDel{Examples of channel resources that can be assigned using decentralized CSP solvers include channel time slots \cite{barcelo2008lba}, frequency channels \cite{duffy2011dcs} and code division multiple access scramble codes \cite{checco2012scs}.}
...
\AzDel{The solution described above is an instance of a decentralized CSP, which provably finds a solution in finite time and exhibits a performance comparable to well known CSP solvers, such as WalkSAT \cite{selman1993lss}.}



\AzDel{\AzCom{This paragraph is a bit out of place!} The solver presented in \cite{duffy2011dcs} accepts two parameters to adjust the learning process in the search for a solution. For particular values of these parameters, it is easy \AzCom{devaluing your past work!} to model the solving process as an MC (see, e.g., \cite{barcelo2010fcc, he2009srb, fang2011dlm}).}
...
\AzDel{It is worth mentioning here that the computation of the transition probabilities from the starting state \AzCom{Are we still talking about particular values of parameters?}\Az{, i.e., when all stations randomly choose the aforementioned resource, boils down to the well-known problem of finding the distribution of the number resources that have been selected by exactly one station \cite{szpankowski1983asc}.
However, the transition probabilities from the other states where only a fraction of the stations randomly select the resource, while the others make a deterministic decision, has not been studied in the literature and is one of the contributions of the current paper.}}

\AzDel{Furthermore, we extend the Markov Chain to account for the presence of errors, taking into account that the contending stations, after an unsuccessful transmission, cannot differentiate between a collision or a channel error.
In the following section, we detail the reference scenario and the reference protocol that are the CSP problem and decentralized CSP solver which we consider in the remainder of the paper.}
...
%An assignment $\mathbf{x}$ is a solution to the problem if all the constraints are satisfied.
%
%In radio resource allocation problems, it is natural that each of the variables represents one of the wireless stations, and that $\mathbf{B}$ represents the available resources.
%It is likely that the different stations cannot communicate until the CSP is solved.
%Therefore, it is necessary to treat the problem as a decentralized CSP.
...
%The distributed CSP solver is executed in each of the stations and the only information available to the algorithm is whether all the clauses containing the wireless station are satisfied or not.
%As detailed in \cite{duffy2011dcs}, previous CSPs are decentralized.

%While there are solvers for this kind of problem (e.g., \cite{selman1993lss}), they require the presence of a centralized
%are typically centralized \AzCom{references?}, i.e., assuming that the solver has the complete information about all variables. In communication networks, however, this assumption normally does not hold. Examples are often encountered in the area of wireless networks where nodes typically function in the absence of any centralized control. Hence, it is necessary to find solvers that can solve the constraint satisfaction problem in a decentralized manner.
...
%The decentralized CSP solver of \cite{duffy2011dcs} stochastically searches for a solution of the problem using an iterative approach.
%%The solver learns from previous mistakes in order to increase the chances of reaching a solution in the next step.
%In the first step, the variables take one of the possible values with equal probability. Then the clauses are evaluated and this information is used to determine the probabilities for the assignments of values to the variables in the next round.
%The solver uses two different parameters, $a$ and $b$, to adjust the process.
...
%A curiosity is that the state $S_{N-1}$, in which a single station collides, can never be reached.
%\begin{figure}
%  \psfrag{S0}[cc][cc]{$S_0$}
%  \psfrag{S1}[cc][cc]{$S_1$}
%  \psfrag{S2}[cc][cc]{$S_2$}
...
\end{align}
Finally, the transition probabilities for $d<N$ can be calculated by replacing $S(j)$ in (\ref{eq:Pij}). \Az{When $d=0$, i.e., when all the $N$ stations randomly select a slot, this result exactly matches the one obtained in \cite{szpankowski1983asc}.}
...
\Az{In this case, the probability of moving from the state $S_d$ to the state $S_\delta$ is the probability that $i \in \left[ \delta, N\right]$ stations do not collide, but exactly $i-\delta$ of those stations suffer a channel error, i.e.,
\begin{equation}
