...
And the solution described above is an instance of a decentralized CSP, which provably finds a solution in finite time and exhibits a performance comparable to well known constraint satisfaction problem solvers such as WalkSAT \cite{selman1993lss}.
...
The first example consists in a set of WLAN access points that need to select the channels they use in a distributed fashion. Since the access points that are close to one another may interfere with each other's communication, but may not be able to communicate directly with each other, this presents a decentralized CSP problem. Here variable $x_i$ indicates the channel for access point $i$, chosen from the set of available channels $\mathbf{B}$. There is one clause per every pair of access points in each other's interference range, indicating whether they have selected the same channel or not.
